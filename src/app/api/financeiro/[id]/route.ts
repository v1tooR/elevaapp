...
async function requireFinanceAdmin(supabase: ServerSupabaseClient, user: { id: string }) {
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!await requireFinanceAdmin(supabase, user)) return NextResponse.json({ error: 'Acesso negado' }, { status: 403 })
